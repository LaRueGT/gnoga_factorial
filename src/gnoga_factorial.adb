...
procedure gnoga_factorial is
   ...
   procedure On_Factorial (Object : in out Gnoga.Gui.Base.Base_Type'Class) is
      ...
      function Factorial (N : Natural) return String is
   ...
   begin
      if not Dec_Check_Box.Checked then
         Result_Label.Text ("Result " & Input_Text.Value & "!= " &
                              Long_Long_Integer'Image (Factorial (Natural'Value
                              (Input_Text.Value))));
      else
         Result_Label.Text (
                            "Result " & Input_Text.Value & "!= " & 
                              Factorial (Natural'Value (Input_Text.Value)));
      end if;
   exception
      when others =>
   ...
   procedure Dec_Range_Change 
     (Element : in out Gnoga.Gui.Base.Base_Type'Class) is
...
begin
   Gnoga.Application.Title ("gnoga_factorial");
   ...
   Dec_Range_Label.Create (Fact_Form, Dec_Range, " decimals", False);
   Dec_Range.On_Change_Handler(Dec_Range_Change'Unrestricted_Access);
   
   Main_View.Put_Line ("Calculate the factorial");
   Fact_Button.Create (Main_View, "Calculate");
   Fact_Button.On_Click_Handler (On_Factorial'Unrestricted_Access);
   Fact_Button.On_Submit_Handler (On_Factorial'Unrestricted_Access);
